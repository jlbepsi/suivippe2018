...
if ($nbp==5){//pour 57 activit�s, parcours indiff�renci� (1er semestre)
...
$pdf->SetTitle("Tableau de synth�se");
...
$txt="BTS SERVICES INFORMATIQUES AUX ORGANISATIONS � TABLEAU DE SYNTH�SE";
$pdf->Cell(0,$hvt,$txt,0,1,"C");
$pdf->SetFont($police,'B',$potexte);
$txt="Nom et pr�nom du candidat : ".$ident[0]["nom"]." ".$ident[0]["prenom"];
$pdf->Cell(60, $hvt,"",0,0);
$pdf->Cell(160,$hvt,$txt,0,0);
$txt="Parcours : ".$ident[0]["nomenclature"];
$pdf->Cell(60,$hvt,$txt,0,0);
$txt="Num�ro du candidat : ".$ident[0]["numexam"];
...
$x=$x0+$lgoblig/2;;
$y0=$pdf->getY();
$y1=$y0+$hv;//hauteur texte � 90�
$pdf->SetFillColor($grisclairfond);

for ($i=0;$i<4;$i++){ //4 s�r, sinon on est mal
...
for ($src=0 ; $src<3 ; $src++){//0=formation / 1=stage1 / 2=stage2
	...
	foreach($corps as $situ){
	  ...
	  //affichage des X pour les activites cit�es
	  $pdf->SetFont($police,'B',$pocroix);
...
$txt="certifie que le candidat (la candidate) a bien effectu� en formation les activit�s et missions pr�sent�es dans ce tableau";
$pdf->Cell(0,$hvt,$txt,0,1,"R");
$pdf->Cell(80,$hvt,$txtj,0,0);
$txt="repr�sentant-e de l�organisation";
$pdf->Cell(120,$hvt,$txt,0,0);
$txt="certifie que le candidat (la candidate) a bien effectu� en stage les activit�s et missions pr�sent�es dans ce tableau";
$pdf->Cell(0,$hvt,$txt,0,1,"R");
